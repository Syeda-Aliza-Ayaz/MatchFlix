...
CREATE TABLE movies (
    ...
    -- Dimensional scores (aggregate/editorial, not per-user)
    emotional_impact    NUMERIC(4,2) DEFAULT 5.0,
    cinematography      NUMERIC(4,2) DEFAULT 5.0,
    audio_design        NUMERIC(4,2) DEFAULT 5.0,
    narrative_coherence NUMERIC(4,2) DEFAULT 5.0,
    moral_conflict      NUMERIC(4,2) DEFAULT 5.0,
    thematic_depth      NUMERIC(4,2) DEFAULT 5.0,
    pacing              NUMERIC(4,2) DEFAULT 5.0,
    rewatch_value       NUMERIC(4,2) DEFAULT 5.0,
    mood_tags           TEXT[] DEFAULT '{}',
    mbti_affinity       JSONB DEFAULT '{}',
...
COMMENT ON COLUMN users.updated_at    IS 'Update via trigger — see SECTION 1';
...
COMMENT ON COLUMN movies.synopsis   IS 'Long-form text field (CLOB equivalent)';
COMMENT ON COLUMN movies.mood_tags  IS 'Array of mood/vibe tags e.g. {dark, thought-provoking}';
COMMENT ON COLUMN movies.mbti_affinity IS 'JSON map of MBTI type to affinity score e.g. {"INTJ": 0.9}';
...
COMMENT ON COLUMN compatibility_cache.is_stale          IS 'TRUE = needs background recalculation';
...
COMMENT ON TABLE  session_shared_movies IS 'Normalised junction table replacing top_shared_movie_1/2/3 columns';
...
COMMENT ON COLUMN solo_recommendations.shown_to_user       IS 'TRUE = displayed in final top 3 to user';
...
CREATE POLICY "Allow public read access" ON public.movie_genre_map
    FOR SELECT USING (true);

-- Dimension weights
CREATE POLICY "Users can view their own weights" ON public.user_dimension_weights
    FOR SELECT USING (true);

CREATE POLICY "Users can manage their own weights" ON user_dimension_weights
    FOR ALL USING (
...
-- Solo sessions
CREATE POLICY "Users can manage their own sessions" ON solo_sessions
    FOR ALL USING (
        user_id IN (SELECT user_id FROM users WHERE email = auth.jwt() ->> 'email')
    );
